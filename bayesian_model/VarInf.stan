...
generated quantities {
  // Only compute these for model checking runs, not VI production runs.
  // Comment out log_lik and y_rep when running variational() to avoid
  // computing N=200K quantities on every draw sample.
  ...
  for (n in 1:N) {
    ...
    real gamma_n   = gamma_base + gamma_fast_delta * is_fast[n];
    real p_correct = gamma_n + (alpha - gamma_n)
                   * inv_logit(skill_eff - question_difficulty[q]);
    p_correct      = fmin(fmax(p_correct, 1e-6), 1.0 - 1e-6);
